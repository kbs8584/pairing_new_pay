...
public class ProcWebHookAllat {
	...
	public void exec(RoutingContext rc,SharedMap<String,Object> sharedMap) {
		...
		if(!requestMap.getString("product_cd").equals("")){
			requestMap.put("tmnId",requestMap.getString("product_cd"));
		}
		
		if(requestMap.isEquals("sign", "-")){
		...
		logger.info("payType: {}",requestMap.getString("pay_type"));
		logger.info("shop_id: {}",requestMap.getString("shop_id"));
		logger.info("order_no:{}",requestMap.getString("order_no"));
		logger.info("tmnId  : {}",requestMap.getString("tmnId"));
		logger.info("tx_seq_no: {}",requestMap.getString("tx_seq_no"));
		logger.info("authCd : {}",requestMap.getString("approval_no"));
		logger.info("card_no: {}",requestMap.getString("card_no"));
		logger.info("approved: {}",requestMap.getString("approval_ymdhms"));
		logger.info("refund : {}",requestMap.getString("cancel_ymdhms"));
		//REFUND 원거래번호는 tx_seq_no 로 처리한다.
		...
		if(requestMap.getString("trxType").equals("REFUND")){
			...
			requestMap.put("regDate",requestMap.getString("cancel_ymdhms"));
		}else{
			requestMap.put("amount", requestMap.getLong("amt"));
			requestMap.put("regDate",requestMap.getString("approval_ymdhms"));
		}
		...
		sharedMap.put("regDate"		, requestMap.getString("regDate"));
		sharedMap.put("payerName" 	, requestMap.getString("buyer_nm"));
		sharedMap.put("payerEmail" 	, requestMap.getString(""));
		sharedMap.put("payerTel" 	, requestMap.getString(""));
		
		
		
		
		if(requestMap.isEquals("trxType", "REFUND")){
			sharedMap.put("vanRootTrxId", requestMap.getString("tx_seq_no"));
			sharedMap.put("rootTrnDay"	, requestMap.getString("approval_ymdhms").substring(0,8));
	...
	private boolean valid() {
		...
		int install = CommonUtil.parseInt(requestMap.getString("sell_mm"));
		//할부기간 설정 
		sharedMap.put("installment",CommonUtil.zerofill(install,2));
		...
		if(requestMap.getString("trxType").equals("REFUND")) {
			...
			logger.info("취소요청:{},원거래금액:{}",sharedMap.getLong("amount") , -rootTrxMap.getLong("amount"));
			...
			String number = requestMap.getString("card_no").replaceAll("-", "");
			
			sharedMap.put("bin"		, number.substring(0,6));
			sharedMap.put("last4"	, number.substring(number.length()-4));
			...
			sharedMap.put("cardType", binMap.getString("type"));
