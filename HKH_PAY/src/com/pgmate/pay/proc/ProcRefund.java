...
*/		
		...
		// 노티스케쥴러에서 예외처리로 처리 변경
	/*	
		// 2022-09-15 - noti 취소건은 노티스케쥴러에서 처리
		if (!sharedMap.getString(PAYUNIT.TRX_ID).substring(0, 1).equals("N")) {
			trxDAO.updateTrxRFD(sharedMap, response);
		} else {
			trxDAO.updateTrxRFDnotify(sharedMap, response);
			logger.debug("========== exec() - trxDAO.updateTrxRFDnotify - nofi 취소건 날짜,시간 DB UPDATE : " + sharedMap.getString(PAYUNIT.TRX_ID));
		}
	*/	
		...
		/*	
			...
			if (!sharedMap.getString(PAYUNIT.TRX_ID).substring(0, 1).equals("N")) {
				...
				logger.debug("========== exec() - trxDAO.updateForceRefund - nofi 취소건은 DB UPDATE 하지 않음. : " + sharedMap.getString(PAYUNIT.TRX_ID));
			}
		*/
			...
			if (riskChk) {
				logger.info("========== exec() - 원거래 전취소 정산보류 해제 되었습니다.");
			} else {
...
*/		
	...
	public void valid2(Request request, SharedMap<String, Object> sharedMap, SharedMap<String, Object> sales_mchtMap, SharedMap<String, Object> sales_mchtTmnMap, SharedMap<String, Object> sales_mchtMngMap) {
		...
		request.refund.tmnId  = sales_mchtTmnMap.getString("tmnId");
		//20190710 과세 유형에 따른 부가세 처리 
		...
		if (CommonUtil.isNullOrSpace(request.refund.rootTrxId)) {
			logger.info("========== valid2() - request.refund.rootTrxId : " + request.refund.rootTrxId);
			logger.info("========== valid2() - TRACK_ID     : {}", request.refund.trackId);
			logger.info("========== valid2() - ROOT_TRACK_ID: {}", request.refund.rootTrackId);
			logger.info("========== valid2() - ROOT_TRX_DAY : {}", request.refund.rootTrxDay);
			...
			logger.info("========== valid2() - ROOT_TRX_ID  : {},{}", request.refund.tmnId, request.refund.rootTrxId);
			trxMap = trxDAO.getTrxPayByTrxId(request.refund.tmnId, request.refund.rootTrxId);
		}
		...
		logger.info("========== valid2() - ROOT_TRX_ID: {}", trxMap.getString("trxId"));
		logger.info("========== valid2() - ROOT_AMOUNT: {}", trxMap.getLong("amount"));
		logger.info("========== valid2() - ROOT_TRX_DAY: {}", trxMap.getString("regDay"));
		logger.info("========== valid2() - ROOT_VAN: {}", trxMap.getString("van"));
		logger.info("========== valid2() - ROOT_VANID: {}", trxMap.getString("vanId"));
		...
		SharedMap<String, Object> salesMap = new SharedMap<String, Object>();
		if (CommonUtil.isNullOrSpace(trxMap.getString("salesId"))) {
			logger.info("========== valid2() -  salesId IS NULL");
		} else {
			logger.info("========== valid2() - trxMap salesId : " + trxMap.getString("salesId"));
			salesMap = trxDAO.getSalesMngByIdChk(trxMap.getString("salesId"));
			if (salesMap == null) {
				logger.info("========== valid2() - 영업사원 아이디가 존재하지 않습니다. : trxMap salesId : " + trxMap.getString("salesId"));
				response.result = ResultUtil.getResult("9999", "영업사원결제", "영업사원 아이디가 존재하지 않습니다.");
				return;
			} else {
				if (CommonUtil.isNullOrSpace(salesMap.getString("salesId"))) {
					logger.info("========== valid2() - salesMap salesId = empty ");
					logger.info("========== valid2() END");
					logger.debug("========== valid2() - 영업사원 아이디가 존재하지 않습니다. : salesMap salesId : " + salesMap.getString("salesId"));
					response.result = ResultUtil.getResult("9999", "영업사원결제", "영업사원 아이디가 존재하지 않습니다.");
					return;
				} else {
		...
		if (sales_mchtMngMap.isEquals("payStatus", "중지")) {
			if (request.refund.metadata != null) {
				if (request.refund.metadata.isEquals("grade", "본사") || request.refund.metadata.isEquals("grade", "대행사")) {
					...
					logger.debug("결제 중지 가맹점 payStatus : {},{}", sales_mchtMngMap.getString("payStatus"));
					response.result = ResultUtil.getResult("9999", "취소거절", "승인중지된 가맹점입니다. 취소는 관리자에게 문의하여 주시기 바랍니다.");
					return;
				}
			} else {
		...
		if (!sales_mchtMap.isEquals("status", "사용")) {
			if (request.refund.metadata != null) {
				if (request.refund.metadata.isEquals("grade", "본사") || request.refund.metadata.isEquals("grade", "대행사")) {
					logger.debug("========== valid2() - status 중지 가맹점 본사 계정으로 접속하여 허용함. grade : {}", request.refund.metadata.getString("grade"));
				} else {
					logger.debug("========== valid2() - 결제 중지 가맹점 status : {}", sales_mchtMap.getString("status"));
					response.result = ResultUtil.getResult("9999", "취소거절", "승인중지된 가맹점입니다. 취소는 관리자에게 문의하여 주시기 바랍니다.");
					return;
				}
			} else {
		...
		SharedMap<String,Object> refundedMap = trxDAO.getTrxRefundSumByTrxId(trxMap.getString("trxId"));
		long refundedAmount = refundedMap.getLong("AMT");
		logger.info("========== valid2() - REFUNDED_AMT: {}",refundedAmount);
		...
		if (request.refund.amount == trxMap.getLong("amount")) {
			...
			logger.info("========== valid2() - RFD_SEQ   : {}", sharedMap.getString("rfdAllCnt"));
		}
...
logger.info("========== valid2() - today :  " + CommonUtil.getCurrentDate("yyyyMMdd"));
logger.info("========== valid2() - trxMap 원거래 regDay :  " + trxMap.getString("regDay"));
logger.info("========== valid2() - 취소권한 refundRule :  " + sales_mchtMngMap.getString("refundRule"));
		SharedMap<String, Object> mchtInfo = trxDAO.getMchtInstall(sales_mchtMngMap.getString("mchtId"));
		
		if (!trxMap.getString("regDay").equals(CommonUtil.getCurrentDate("yyyyMMdd"))) {
			String forceType = trxDAO.getForceRefund(trxMap.getString("trxId"));
			if (CommonUtil.isNullOrSpace(forceType)) {
				logger.info("");
				logger.info("========== valid2() - 취소권한 기존 체크" );
				if (sales_mchtMngMap.isEquals("refundRule", "MID")) {
					long notSettleSum = trxDAO.notSettleSum(sales_mchtMngMap.getString("mchtId"));
					logger.info("========== valid2() - MID 미정산금액  : {}",notSettleSum);
					if (notSettleSum < request.refund.amount) {
						trxDAO.insertRefundIQR(trxMap.getString("trxId"),"가맹점 취소금액이 미정산 금액보다 적습니다. 미정산금액:"+notSettleSum+" ,취소요청:"+request.refund.amount);
						response.result = ResultUtil.getResult("9999", "취소실패","가맹점 취소금액이 미정산 금액보다 적습니다."); return;
					}
				} else if(sales_mchtMngMap.isEquals("refundRule", "TID")) {
					
					SharedMap<String,Object> rootTrxMap = trxDAO.getTrxCapDtlByTrxId(trxMap.getString("trxId"));
					if (rootTrxMap.isEquals("stlStatus", "정산대기") && !rootTrxMap.isEquals("stlDay", CommonUtil.getCurrentDate("yyyyMMdd"))) {
						logger.info("========== valid2() - TID 취소 가능거래 정산상태 :{},정산예정일:{}",rootTrxMap.getString("stlStatus"),rootTrxMap.getString("stlDay"));
					} else if(rootTrxMap.isEquals("stlStatus", "정산대기") && rootTrxMap.isEquals("stlDay", CommonUtil.getCurrentDate("yyyyMMdd"))) {
						if (CommonUtil.parseInt(CommonUtil.getCurrentDate("HH")) < 10){
							logger.info("========== valid2() - TID 취소 가능거래 9시 이전거래 정산상태 :{},정산예정일:{}",rootTrxMap.getString("stlStatus"),rootTrxMap.getString("stlDay"));
						} else {
							response.result = ResultUtil.getResult("9999", "취소실패","당일 정산건으로 취소가 불가합니다.관리자에게 문의하여 주시기 바랍니다."); 
							return;
						}
					...
					*/
					...
					// 해당 가맹점은 당일거래가 아니면 취소 불가. 2019.06.20
				} else if (sales_mchtMngMap.isEquals("refundRule", "MID2")) {
					SharedMap<String,Object> rootTrxMap = trxDAO.getTrxCapDtlByTrxId(trxMap.getString("trxId"));
					if ((rootTrxMap.isEquals("stlStatus", "정산완료")) || Long.parseLong(CommonUtil.getCurrentDate("yyyyMMdd")) >= rootTrxMap.getLong("stlDay") ) {
						response.result = ResultUtil.getResult("9999", "취소실패", "당일 정산건으로 취소가 불가합니다.관리자에게 문의하여 주시기 바랍니다.");
						return;
				...
				logger.info("========== valid2() - 강제취소 : {}",forceType);
			}
		}
		...
		// 2021-11-19 취소권한 추가		
		// 취소권한 : 없음
		logger.info("========== valid2() - 취소권한 체크 : " + mchtInfo.getString("refundRule"));
		if (!request.refund.metadata.isEquals("grade", "본사")) {
			if (mchtInfo.isEquals("refundRule", "NLID")) {
				logger.info("========== valid2() - 취소권한 : 없음 : " + mchtInfo.getString("refundRule") + CommonUtil.getCurrentDate("yyyyMMdd") );
				response.result = ResultUtil.getResult("9999", "취소실패", "취소권한이 없어 취소가 불가합니다. 관리자에게 문의하여 주시기 바랍니다.");
				return;
			// 취소권한 당일취소	
			} else if (mchtInfo.isEquals("refundRule", "TLID")) {
				...
				if ("".equals(rootTrxId)) {
					rootTrxMap = trxDAO.getTrxCapDtlByTrxId(trxMap.getString("trxId"));
					logger.info("========== valid2() - 취소권한 : 당일취소 : " + mchtInfo.getString("refundRule") + "" +  CommonUtil.getCurrentDate("yyyyMMdd") );
					logger.info("========== valid2() - rootTrxMap trxDay : " + rootTrxMap.getString("trxDay") );
					if (!rootTrxMap.isEquals("trxDay", CommonUtil.getCurrentDate("yyyyMMdd"))) {
						System.out.println();
						System.out.println("rootTrxMap trxDay : " + rootTrxMap.get("trxDay"));
						System.out.println("VS yyyyMMdd : " + CommonUtil.getCurrentDate("yyyyMMdd"));
			...
			// 취소권한 PG정산일전
				...
				if ("".equals(rootTrxId)) {
					rootTrxMap = trxDAO.getTrxCapDtlByTrxId(trxMap.getString("trxId"));
					logger.info("========== valid2() - 취소권한 : PG정산일전 : " + mchtInfo.getString("refundRule") + CommonUtil.getCurrentDate("yyyyMMdd") );
					logger.info("========== valid2() - rootTrxMap stlVanDay : " + rootTrxMap.getString("stlVanDay") );
					if (Long.parseLong(CommonUtil.getCurrentDate("yyyyMMdd")) >= rootTrxMap.getLong("stlVanDay") ) {
						response.result = ResultUtil.getResult("9999", "취소실패", "PG정산일전이 아니므로 취소가 불가합니다. 관리자에게 문의하여 주시기 바랍니다.");
						return;
					}
				...
				if ("".equals(rootTrxId)) {
					rootTrxMap = trxDAO.getTrxCapDtlByTrxId(trxMap.getString("trxId"));
					logger.info("========== valid2() - 취소권한 : 가맹점정산일전 : " + mchtInfo.getString("refundRule") + CommonUtil.getCurrentDate("yyyyMMdd") );
					logger.info("========== valid2() - rootTrxMap stlDay : " + rootTrxMap.getString("stlDay") );
					if (Long.parseLong(CommonUtil.getCurrentDate("yyyyMMdd")) >= rootTrxMap.getLong("stlDay")) { 
						response.result = ResultUtil.getResult("9999", "취소실패", "PG정산일전이 아니므로 취소가 불가합니다. 관리자에게 문의하여 주시기 바랍니다.");
						return;
					}
			...
			// 취소권한 : 모두취소
				...
				logger.info("========== valid2() - 취소권한 : 모두취소 : " + mchtInfo.getString("refundRule") + CommonUtil.getCurrentDate("yyyyMMdd") );
			}
		} else {
			logger.info("========== valid2() - 취소처리 grade : " + request.refund.metadata.getString("grade") + CommonUtil.getCurrentDate("yyyyMMdd") );
		}
		
		logger.info("========== valid2() - RFD_ALL   : {}",sharedMap.getString("rfdAll"));
		logger.info("========== valid2() - RFD_TYPE  : {}",sharedMap.getString("rfdType"));
		...
		if (trxResMap != null && trxResMap.getString("pairingRouteVan") != null && !trxResMap.getString("pairingRouteVan").isEmpty()) {
			...
			sharedMap.put("mchtBRN", mchtBRN);
			sharedMap.put("shopDomain", sales_mchtMngMap.getString("shopDomain"));
			sharedMap.put("mchtIp", "");
			sharedMap.put("vanTrxId", trxResMap.getString("vanTrxId"));
		}
/*
