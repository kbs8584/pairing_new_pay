...
public class Smartro implements Van {
	private static Logger logger = LoggerFactory.getLogger(com.pgmate.pay.van.Smartro.class);
	...
	public Smartro(SharedMap<String, Object> tmnVanMap) {
		VAN_ID =  tmnVanMap.getString("vanId").trim();
		CRYPTOKEY	= StringEscapeUtils.unescapeHtml(StringEscapeUtils.unescapeJava(tmnVanMap.getString("cryptoKey").trim()));
		VAN = tmnVanMap.getString("van");
		CANCELPWD = tmnVanMap.getString("secondKey");
	...
	public synchronized SharedMap<String, Object> sales(TrxDAO trxDAO, SharedMap<String, Object> sharedMap, Response response) {
		...
		if (response.pay.metadata != null) { // KSNET 에서 가급적 사용하지 않으려함.
			if (response.pay.metadata.isTrue("cardAuth")) {
				// 구인증 방식
				payClient.setParam("BuyerAuthNum", response.pay.metadata.getString("authDob")); 	// 구 인증 방식만 사용 생년월일
																														// YYMMDD
		...
		try {
			...
			logger.info("SMARTRO CONNECTION ERROR [{}]", CommonUtil.getExceptionMessage(e));
			logger.info("SMARTRO IP :{},PORT:{}", SMARTRO_DOMAIN, SMARTRO_PORT);
		...
		if (res.getParameter("ResultCode").equals("3001")) {
			...
			sharedMap.put("vanResultCd", CommonUtil.nToB(res.getParameter("ResultCode")));
	...
	public synchronized SharedMap<String, Object> refund(TrxDAO trxDAO, SharedMap<String, Object> sharedMap, SharedMap<String, Object> payMap, Response response) { 
		logger.info("========== ========== ========== ========== ========== SMARTRO refund");
		logger.info("========== ========== ========== ========== CANCELPWD : " + CANCELPWD);
		/** 2-5. 서비스모드 설정(결제 서비스 : PY0 , 취소 서비스 : CL0) */
		payClient.setParam("SERVICE_MODE", "CL0");

		payClient.setParam("CancelAmt", CommonUtil.toString(response.refund.amount));
		payClient.setParam("CancelPwd", CANCELPWD); // 취소 패스워드
		//payClient.setParam("CancelPwd", vanMap); // 취소 패스워드 재로드 테스트
		...
		if (res.getParameter("ResultCode").equals("2001")) {
			...
			sharedMap.put("vanDate", res.getParameter("CancelDate") + res.getParameter("CancelTime"));
			
			logger.info("SMARTRO refund CancelDate+CancelTime : " + res.getParameter("CancelDate") + res.getParameter("CancelTime"));
		} else if (!res.getParameter("ResultCode").equals("XXXX")) {
			String vanMessage = res.getParameter("ErrorCD") + " " + res.getParameter("ResultMsg") + " " + res.getParameter("ErrorMsg");
			response.result = ResultUtil.getResult(res.getParameter("ResultCode"), "승인실패", vanMessage);
			sharedMap.put("vanTrxId", res.getParameter("TID"));
	...
	public static void main(String[] args) {
		...
		Smartro sm = new Smartro(tmnVanMap);
